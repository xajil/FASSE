<?php
    ...
    $descripProd=consultasSQL::clean_string($_POST['prod-descrip-price']);
    ...
    if($codeProd!="" && $nameProd!="" && $cateProd!="" && $priceProd!="" && $modelProd!="" && $marcaProd!="" && $stockProd!="" && $codePProd!="" && $descripProd!=""){
        ...
        if($verificaltotal<=0){
            if($imgType=="image/jpeg" || $imgType=="image/png"){
                if(($imgSize/1024)<=$imgMaxSize){
                    ...
                    $imgFinalName2=$codeProd."2".$imgEx;
                    $imgFinalName3=$codeProd."3".$imgEx;
                    if(move_uploaded_file($_FILES['img']['tmp_name'],"../assets/img-products/".$imgFinalName )) {
                      if(move_uploaded_file($_FILES['img2']['tmp_name'],"../assets/img-products/".$imgFinalName2 )) {
                        if(move_uploaded_file($_FILES['img3']['tmp_name'],"../assets/img-products/".$imgFinalName3 )) {
                         if(consultasSQL::InsertSQL("producto", "CodigoProd, NombreProd, CodigoCat, Precio, Descuento, Modelo, Marca, Stock, NITProveedor, Imagen, Nombre, Estado, descripcion,Imagen2,Imagen3", "'$codeProd','$nameProd','$cateProd','$priceProd', '$descProd', '$modelProd','$marcaProd','$stockProd','$codePProd','$imgFinalName','$adminProd', '$estadoProd', '$descripProd','$imgFinalName2','$imgFinalName3'")){
                            echo '<script>
                                swal({

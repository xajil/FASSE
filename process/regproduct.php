<?php
    ...
    if($codeProd!="" && $nameProd!="" && $priceProd!="" && $modelProd!="" && $marcaProd!="" && $stockProd!=""){
            ...
            if($imgType=="image/jpeg" || $imgType=="image/png"){
                if(($imgSize/1024)<=$imgMaxSize){
                    ...
                    if(move_uploaded_file($_FILES['img']['tmp_name'],"../assets/img-products/".$imgFinalName)){
                        if(consultasSQL::InsertSQL("producto", "CodigoProd, NombreProd, Precio, Descuento, Modelo, Marca, Stock, Imagen, Nombre", "'$codeProd','$nameProd','$priceProd', '$descProd', '$modelProd','$marcaProd','$stockProd','$imgFinalName','$adminProd'")){
                            echo '<script>
                                swal({
